...
class TrackEditor:
    ...
    def __init__(self):
        ...
        self.Refresh = Button(self.root)

        self.Refresh['text'] = '    刷新    '

        self.Refresh['command'] = self.Refresh_Loop
    ...
    def Load_Data(self, cell_data_1, cell_data_2, cell_data_3):
        ...
        for row in rows:
            ...
            cell_data_1.append(booksheet.cell(
                row=i, column=1).value)  # 获取第i行1列的数据

            cell_data_2.append(booksheet.cell(
                row=i, column=2).value)  # 获取第i行2列的数据

            cell_data_3.append(booksheet.cell(
                row=i, column=3).value)  # 获取第i行3列的数据
